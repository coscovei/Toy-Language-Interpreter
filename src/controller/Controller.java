...
public class Controller implements IController {
    ...
    public void executeOneStep() throws FileNotFoundException, RepoException, MyException {
        // Executes one step for all current program states.
        // (Mostly for compatibility with previous labs; the menu usually calls executeAllSteps().)
        if (executor == null) {
        ...
        // conservative GC before the step
    ...
    public void executeAllSteps() throws FileNotFoundException, RepoException, MyException {
        ...
        while (!prgList.isEmpty()) {
            ...
            // conservative GC (shared heap; roots from all symTables)
            runGarbageCollector(prgList);
    ...
    // -------- Garbage Collector (safe, transitive) --------

    private void runGarbageCollector(List<PrgState> prgList) {
        if (prgList.isEmpty()) return;

        // heap is shared between prg states (fork shares it)
        MyIHeap<IValue> heap = prgList.get(0).getHeap();
